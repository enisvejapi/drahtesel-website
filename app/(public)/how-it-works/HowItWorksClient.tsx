...
import type { Faq } from '@/lib/data-server'

const DE_FAQS_FALLBACK = [
  { id: 'de1', question: 'Was ist in jeder Miete enthalten?', answer: 'Jede Miete beinhaltet Helm und Schloss. Je nach Fahrrad gibt es auch Korb, Satteltasche, Stadtplan oder Kinderhelme. Schau auf der jeweiligen Fahrradseite nach.' },
  { id: 'de2', question: 'Kann ich meine Buchung stornieren?', answer: 'Ja. Kostenlose Stornierung bis 24 Stunden vor der Abholung. Stornierungen weniger als 24 Stunden vorher sind nicht erstattungsfähig.' },
  { id: 'de3', question: 'Wird eine Kaution verlangt?', answer: 'Für Standard-Cityräder keine Kaution. Für E-Bikes und Lastenräder gilt eine rückerstattbare Kaution von 50–100 €.' },
  { id: 'de4', question: 'Was passiert bei verspäteter Rückgabe?', answer: 'Bitte melde dich so früh wie möglich, wenn du dich verspätest. Späte Rückgaben werden anteilig stündlich berechnet.' },
  { id: 'de5', question: 'Muss ich einen Ausweis mitbringen?', answer: 'Ja, bitte bringe einen gültigen Personalausweis oder Reisepass zur Abholung mit.' },
  { id: 'de6', question: 'Was passiert, wenn das Fahrrad eine Panne hat?', answer: 'Ruf unsere Support-Hotline an. Wir helfen dir so schnell wie möglich oder organisieren Ersatz. Alle Fahrräder werden vor jeder Miete professionell gewartet.' },
]

const EN_FAQS_FALLBACK = [
  { id: 'en1', question: "What's included with every rental?", answer: "Every rental includes a helmet, a quality lock, and a repair kit. Depending on the bike, you may also get a basket, saddlebag, city map, or child helmets. Check the individual bike page for what's included." },
  { id: 'en2', question: 'Can I cancel my booking?', answer: 'Yes. You can cancel free of charge up to 24 hours before your scheduled pickup. Cancellations made less than 24 hours before are non-refundable.' },
  { id: 'en3', question: 'Is there a deposit required?', answer: 'A deposit may be required at pickup depending on the bike type. For standard city bikes, no deposit is required. For e-bikes and cargo bikes, a refundable deposit of €50–€100 applies.' },
  { id: 'en4', question: 'What happens if I return late?', answer: "If you're running late, please call or message us as soon as possible. Late returns are charged at a prorated hourly rate." },
  { id: 'en5', question: 'Do I need to bring ID?', answer: 'Yes, please bring a valid government-issued ID or passport at pickup.' },
  { id: 'en6', question: 'What if the bike breaks down?', answer: 'In the unlikely event of a mechanical issue, call our support line. We will do our best to assist you or arrange a replacement.' },
]
...
export default function HowItWorksClient({ faqs }: Props) {
  const [openFaq, setOpenFaq] = useState<number | null>(null)
  const { locale } = useLocale()
  const de = locale === 'de'

  // Use Supabase FAQs if available, otherwise fall back to hardcoded
  const displayFaqs = faqs.length > 0 ? faqs : (de ? DE_FAQS_FALLBACK : EN_FAQS_FALLBACK)
